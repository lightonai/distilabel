...
- The only requirement for the dataset format is having a source column which is expected to be a string or a list of strings (straight input to the LM) or a list of image paths (which can point straight to jpg/png files or a page in a pdf with format `path/to/pdf_page_x.pdf`). This is atm only an expectation in `VLM._format_input()` when it is passed to `LMGenerationTask.input_formatter`, so you can change the `input_formatter`/override this if you need or just make `VLM._format_input()` more general.
...
- I have implemented a batch level caching system.
  - WARNING! Ensure the cached batches are not full of None values due to earlier failed runs. You will get a warning when `LMGenerationTask` generates batches that have 10% or more rows that failed to format or get a model response. You can use `invalidate_cache` to overwrite them.
