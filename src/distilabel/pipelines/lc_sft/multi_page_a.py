...
def run_pipeline(config: Config):
    ...
    # Build idx→filename for dereferencing hard negatives
...
if __name__ == '__main__':
    ...
    hn = distiset.filter(utils.hf_batched(lambda row: row['split'] == 'hn_short'), batched=True, num_proc=1)
    adj = distiset.filter(utils.hf_batched(lambda row: row['split'] == 'adj_short'), batched=True, num_proc=1)
