...
from distilabel.constants import DISTILABEL_METADATA_KEY, MODEL_COST_INPUT, MODEL_COST_OUTPUT
...
class _Task(_Step, ABC):
    ...
    def format_output(
        ...
        addition the `reasoning` and `input` used to generate the output is also received just in case it's
    ...
    def _format_outputs(
        ...
        for (output, reasoning, stats, extra), input in zip(
            ...
            try:
                ...
                formatted_output = self.format_output(output, reasoning, input)
                ...
                formatted_output = self._add_cost(formatted_output, stats)
    ...
    def _add_cost(self, output: Dict[str, Any], stats: Dict[str, Any]) -> Dict[str, Any]:
        """
        If the model cost is known, add it to the output.
        """
        input_cost = MODEL_COST_INPUT.get(self.llm.model_name, 0.0)
        output_cost = MODEL_COST_OUTPUT.get(self.llm.model_name, 0.0)
        output['cost'] = (
            output.get('cost', 0.0) +
            input_cost * sum(stats['input_tokens']) +
            output_cost * sum(stats['output_tokens'])
        ) / 1e6 # costs are per million tokens
        return output
...
class Task(_Task, Step):
    ...
    def process(self, inputs: StepInput) -> "StepOutput":  # type: ignore
        ...
        n_cache_hits = sum(cache_hit for output in outputs for cache_hit in output.get('cache_hit', []))
        self._logger.info(f"{n_cache_hits}/{len(outputs)} LM-level cache hits")
        task_outputs = []
...
def iterate_generations_with_stats(
    ...
    """
    extra_keys = [
        key for key in outputs.keys() if key not in 
        ("generations", "statistics", "reasoning_generations", "cache_hit")
    ]
    for i, generation in enumerate(outputs["generations"]):
        reasoning_generation = outputs["reasoning_generations"][i] if outputs["reasoning_generations"] else None
        ...
        yield generation, reasoning_generation, stats, extra
